...
printf("\n");
printf("\nTIME: %s",PERSON.time);
printf("\nMEETING WITH: %s",PERSON.name);
printf("\nMEETING AT: %s",PERSON.place);
printf("\nNOTE: %s",PERSON.note);
printf("\n");
}
break;
...
while(num<1||num>8);
fseek(fpte,-sizeof(PERSON),SEEK_CUR);
fwrite(&PERSON,sizeof(PERSON),1,fpte);
fseek(fpte,-sizeof(PERSON),SEEK_CUR);
fread(&PERSON,sizeof(PERSON),1,fpte);
choice=4;
break;
}
}
...
printf("\n\n\t::YOU ENTERED WRONG PASSWORD::YOU ARE NOT ALLOWED TO
ACCESS ANY FILE::\n\n\tPRESS ANY KEY TO GO BACK...");
getch();
return 1;
}
